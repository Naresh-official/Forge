...
└── packages/
    ...
    ├── contracts/          — Protobuf definitions shared between api & builder
...
**gRPC services (`builder.proto`):**
- `BuilderService.Build` — triggers a build on the builder
- `ApiService.BuildStarted` — builder calls this when a job begins; returns repo metadata + GitHub access token
- `ApiService.BuildCompleted` — builder calls this with the build result (image URL / static artifact key, status, etc.)
...
### `@forge/contracts`
Protobuf definitions (`proto/builder.proto`, `proto/health.proto`) and their generated TypeScript bindings (`src/`). Used by both `apps/api` and `apps/builder`.
...
### gRPC
- `api` and `builder` each run their own gRPC server on separate ports (configured via `@forge/config`).
- The `api` acts as a **client** to the `builder` (`BuilderService`) and as a **server** to the `builder` (`ApiService`).
...
| Redis | `builder` | BullMQ job queue |
