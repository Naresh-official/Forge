    ...
    async (job) => {
        ...
        try {
            ...
            switch (detection.strategy) {
                case "node": {
                    ...
                    if (isStaticFramework(detection.framework)) {
                        if (output.outputDirectory === undefined) {
                            throw new Error(
                                "Static build produced no output directory"
                            )
                        }
                        ...
                        report.imageUrl = `${builderConfig.dockerRegistry.url}/${response.repoFullName}`
                ...
                default:
                    throw new Error(
                        `Cannot build project with unknown strategy`
                    )
            }
